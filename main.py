...
def plot_black_scholes_prices():
    ...
    s_values = np.arange(95, 105, 0.01)     # Stock price range (from 95 to 105) [Covers in-the-money, at-the-money, and out-of-the-money cases.]
...
def compare_option_prices():
    ...
    print('Monte Carlo (Strike 95, t = 0.999):', BS_Call_MC(100, 95, 0.06, 0.3, 1, 0.999, 10).calculate_option_price())
    print('Exact (Strike 95, t = 0.999):', BlackScholes(S=100, X=95, r=0.06, sigma=0.3, T=1).call_price(0.999))

    # Scenario 2: Monte Carlo and exact Black-Scholes for T-t = 0.75
    print('Monte Carlo (Strike 95, t = 0.75):', BS_Call_MC(100, 95, 0.06, 0.3, 1, 0.75, 100000).calculate_option_price())
    print('Exact (Strike 95, t = 0.75):', BlackScholes(S=100, X=95, r=0.06, sigma=0.3, T=1).call_price(0.75))
...
def plot_monte_carlo_vs_exact(curr_time):
    ...
    for i in range(1, 100000, 500):  # Iterating from 1 to 100000 in steps of 500
        new_row = pd.DataFrame({'Iter': [i], 'BSc': [BS_Call_MC(100, 95, 0.06, 0.3, 1, curr_time, i).calculate_option_price()]})
    ...
    plt.hlines(BlackScholes(S=100, X=95, r=0.06, sigma=0.3, T=1).call_price(curr_time), xmin=0, xmax=100000, linestyle='dotted', colors='red', label='Exact')
    ...
    plt.ylim(BlackScholes(S=100, X=95, r=0.06, sigma=0.3, T=1).call_price(curr_time) - 1, BlackScholes(S=100, X=95, r=0.06, sigma=0.3, T=1).call_price(curr_time) + 1)
...
def plot_monte_carlo_vs_exact_for_strikes():
    ...
    for i in t:
        new_row = pd.DataFrame({
            't': [i],
            '95': [BS_Call_MC(100, 95, 0.06, 0.3, 1, i, 100000).calculate_option_price()],
            '98': [BS_Call_MC(100, 98, 0.06, 0.3, 1, i, 100000).calculate_option_price()],
            '100': [BS_Call_MC(100, 100, 0.06, 0.3, 1, i, 100000).calculate_option_price()],
            '105': [BS_Call_MC(100, 105, 0.06, 0.3, 1, i, 100000).calculate_option_price()]
        })
        df = pd.concat([df, new_row], ignore_index=True)
    ...
    plt.plot(t, BlackScholes(S=100, X=95, r=0.06, sigma=0.3, T=1).call_price(t), alpha=0.6, label='X = 95')
    plt.plot(t, BlackScholes(S=100, X=98, r=0.06, sigma=0.3, T=1).call_price(t), alpha=0.6, label='X = 98')
    plt.plot(t, BlackScholes(S=100, X=100, r=0.06, sigma=0.3, T=1).call_price(t), alpha=0.6, label='X = 100')
    plt.plot(t, BlackScholes(S=100, X=105, r=0.06, sigma=0.3, T=1).call_price(t), alpha=0.6, label='X = 105')
